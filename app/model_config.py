...
class ModelClient:
    ...
    def _local(self, system: str, user: str) -> str:
        ...
        if self.model_id not in _local_pipelines:
            try:
                from transformers import pipeline, AutoTokenizer, AutoModelForCausalLM
                import torch

                print(f"[ModelClient] Loading {self.model_id} locally (first load ~1GB)...")
                ...
                _local_pipelines[self.model_id] = {"pipe": pipe, "tokenizer": tokenizer}
                print(f"[ModelClient] ✅ {self.model_id} loaded and cached.")
            except ImportError:
                raise RuntimeError(
        ...
        cached = _local_pipelines[self.model_id]
        pipe = cached["pipe"]
